...
elseif (Test-Path $iqsharpNugetPackagePath -PathType Leaf) {
    ...
    dotnet tool install Microsoft.Quantum.IQSharp --version 0.28.302312-beta --tool-path $Env:TOOLS_DIR --add-source $Env:NUGET_OUTDIR | Write-Host
} else {    
    Write-Host "Installing the $nugetVersion published IQ# dotnet tool"
    dotnet tool install Microsoft.Quantum.IQSharp --version 0.28.302312-beta --tool-path $Env:TOOLS_DIR | Write-Host
