...
Write-Host "##[info] Installing Python prerequisites"
pip install --user `
...
if (Test-Path $iqsharpNugetPackagePath -PathType Leaf) {
    ...
    try {
        ...
        if ($currentInstalledVersion -and ($currentInstalledVersion -ne $Env:NUGET_VERSION)) {
            Write-Host "##[info] IQ# dotnet tool is already installed with a different version."
            Write-Host "##[info] Uninstalling version $currentInstalledVersion"
            dotnet tool uninstall Microsoft.Quantum.IQSharp --tool-path $Env:TOOLS_DIR | Write-Host
        }
    } catch {}

    Write-Host "##[info] Installing the IQ# dotnet tool specific version from the build drop folder."
    Write-Host "##[info]   Version: $Env:NUGET_VERSION"
    Write-Host "##[info]   Source: $Env:NUGET_OUTDIR"
    ...
    Write-Host "##[info] Installing the latest published IQ# dotnet tool"
    dotnet tool install Microsoft.Quantum.IQSharp --tool-path $Env:TOOLS_DIR | Write-Host
}

Write-Host "##[info] Installing the IQ# Kernel from the installed dotnet tool"
try {
    ...
    Write-Host "##[info] IQ# kernel installed ($LastExitCode)"
} catch {
    Write-Host ("##[info] IQ# installation threw error: " + $_)
    Write-Host ("##[info] IQ# might not be correctly installed.")
} finally {
...
if (Test-Path $qsharpPythonWheelPath -PathType Leaf) {
    Write-Host "##[info] Installing the qsharp Python wheel from the build drop folder."
    Write-Host "##[info]   Wheel: $qsharpPythonWheelPath"
    Write-Host "##[info]   Source: $Env:PYTHON_OUTDIR"
    pip install --user --verbose --no-index --find-links=$Env:PYTHON_OUTDIR "qsharp==$Env:PYTHON_VERSION" | Write-Host
} else {
    Write-Host "##[info] Installing the latest published qsharp Python package"
