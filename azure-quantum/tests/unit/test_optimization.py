...
class TestProblem(QuantumTestBase):
    ...
    def test_problem_fixed_variables(self):
        ...
        self.assertEqual([Term(c=10, indices=[0]), Term(c=-5, indices=[])],
                         problem.set_fixed_variables({'1': 1, '2': 1}).terms)

        # test all const terms get merged
        self.assertEqual([Term(c=5, indices=[])],
    ...
    def test_problem_large(self):
        ...
        # create 1 million dummy terms
        problem.add_terms([Term(indices=[9999], c=1.0)]*int(1e6))
...
class TestSolvers(QuantumTestBase):
    ...
    def test_input_params(self):
        ...
        p3_params = ParallelTempering(ws,
                                      sweeps=2024,
                                      all_betas=[3, 4, 5]).params
        self.assertEqual({"all_betas": [3, 4, 5],
                          "replicas": 3,
                          "sweeps": 2024},
                         p3_params["params"])

        s3_params = SimulatedAnnealing(ws, beta_start=3.2, sweeps=12).params
        self.assertEqual({"beta_start": 3.2,
                          "sweeps": 12},
                         s3_params["params"])
    ...
    def test_SimulatedAnnealing_input_params(self):
        ...
        self.assertEqual("microsoft.simulatedannealing-parameterfree.fpga",
                         good.target)
        self.assertEqual({"timeout": 1011, "seed": 4321},
                         good.params["params"])
    ...
    def test_PopulationAnnealing_input_params(self):
        ...
        good = PopulationAnnealing(ws,
                                   ...
                                   beta=beta)
        self.assertIsNotNone(good)
        self.assertEqual("microsoft.populationannealing.cpu",
                         good.target)
        ...
        self.assertEqual({"type": "linear", "initial": 0.8, "final": 5.8},
    ...
    def test_SubstochasticMonteCarlo_input_params(self):
        ...
        good = SubstochasticMonteCarlo(ws,
                                       alpha=alpha,
                                       seed=1888,
        ...
        self.assertEqual({"type": "geometric", "initial": 1.8, "final": 2.8},
        ...
        self.assertEqual({"type": "linear", "initial": 2.8, "final": 15.8},
                         good.params["params"]["beta"])
    ...
    def test_SSMC_bad_input_params(self):
        ...
        self.assertTrue('"schedule_type" can only be'
        ...
        bad_solver = None
        with self.assertRaises(ValueError) as context:
            bad_solver = SubstochasticMonteCarlo(ws,
                                                 alpha=alpha,
                                                 seed=1888,
                                                 target_population=3000,
                                                 step_limit=1000,
                                                 steps_per_walker=5,
                                                 beta=beta)
        self.assertTrue('can only be from class "RangeSchedule"!'
                        in str(context.exception))
