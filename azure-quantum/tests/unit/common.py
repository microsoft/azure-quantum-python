...
class QuantumTestBase(ReplayableTest):
    ...
    def __init__(self, method_name):
        ...
        regex_replacer.register_regex(
            f"https://login.microsoftonline.com/{GUID_REGEX_PATTERN}",
    ...
    def _custom_request_query_matcher(cls, r1: VcrRequest, r2: VcrRequest):
        ...
        assert url1.hostname == url2.hostname, \
               f"hostname: {url1.hostname} != {url2.hostname}"
        assert url1.path == url2.path, \
               f"path: {url1.path} != {url2.path}"
        ...
        for key in q1:
            ...
            assert q1[key][0].lower() == q2[key][0].lower(), \
                   f"&{key}: {q1[key][0].lower()} != {q2[key][0].lower()}"
