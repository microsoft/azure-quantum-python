...
class TestWorkspace(QuantumTestBase):
    ...
    def test_load_workspace_from_arg_success(self):
        """Test successful workspace discovery via ARG with only workspace name."""
        mock_arg_client = mock.Mock(spec=ResourceGraphClient)
        ...
        with mock.patch.dict(os.environ):
            ...
            ws = Workspace(
                name=WORKSPACE,
                **{Workspace._RG_CLIENT_PARAM: mock_arg_client}
            )
            ...
            mock_arg_client.resources.assert_called_once()
            call_args = mock_arg_client.resources.call_args[0][0]
            self.assertIn(f"name =~ '{WORKSPACE}'", call_args.query)
    
    def test_load_workspace_from_arg_with_location(self):
        """Test workspace discovery via ARG with workspace name and location."""
        mock_arg_client = mock.Mock(spec=ResourceGraphClient)
        mock_response = mock.Mock()
        mock_response.data = [{
            'subscriptionId': SUBSCRIPTION_ID,
        ...
        with mock.patch.dict(os.environ):
            ...
            call_args = mock_arg_client.resources.call_args[0][0]
            self.assertIn(f"name =~ '{WORKSPACE}'", call_args.query)
            self.assertIn(f"location =~ '{LOCATION}'", call_args.query)
    
    def test_load_workspace_from_arg_with_resource_group(self):
        """Test workspace discovery via ARG with workspace name and resource group."""
        mock_arg_client = mock.Mock(spec=ResourceGraphClient)
        mock_response = mock.Mock()
        mock_response.data = [{
            'subscriptionId': SUBSCRIPTION_ID,
        ...
        with mock.patch.dict(os.environ):
            ...
            call_args = mock_arg_client.resources.call_args[0][0]
            self.assertIn(f"name =~ '{WORKSPACE}'", call_args.query)
            self.assertIn(f"resourceGroup =~ '{RESOURCE_GROUP}'", call_args.query)
    
    def test_load_workspace_from_arg_with_subscription(self):
        """Test workspace discovery via ARG with workspace name and subscription."""
        mock_arg_client = mock.Mock(spec=ResourceGraphClient)
        mock_response = mock.Mock()
        mock_response.data = [{
            'subscriptionId': SUBSCRIPTION_ID,
        ...
        with mock.patch.dict(os.environ):
            ...
            # Verify subscription was passed to ARG query
            call_args = mock_arg_client.resources.call_args[0][0]
            self.assertIn(f"name =~ '{WORKSPACE}'", call_args.query)
            self.assertEqual(call_args.subscriptions, [SUBSCRIPTION_ID])
    
    def test_load_workspace_from_arg_no_results(self):
        """Test ARG returns no matching workspaces."""
        mock_arg_client = mock.Mock(spec=ResourceGraphClient)
        mock_response = mock.Mock()
        mock_response.data = []
        mock_arg_client.resources.return_value = mock_response
        ...
        with mock.patch.dict(os.environ):
            ...
            self.assertIn("No matching workspace found", str(context.exception))
            self.assertIn(WORKSPACE, str(context.exception))
    
    def test_load_workspace_from_arg_multiple_results_no_location(self):
        """Test ARG returns multiple workspaces and no location specified."""
        mock_arg_client = mock.Mock(spec=ResourceGraphClient)
        mock_response = mock.Mock()
        mock_response.data = [
            {
            ...
            },
            {
                ...
                'location': 'westus',
                'endpointUri': 'https://westus.quantum.azure.com',
                'name': WORKSPACE
            }
        ]
        mock_arg_client.resources.return_value = mock_response
        ...
        with mock.patch.dict(os.environ):
            ...
            self.assertIn("Multiple Azure Quantum workspaces found", str(context.exception))
            self.assertIn("Please specify location", str(context.exception))
    
    def test_load_workspace_from_arg_multiple_results_with_location(self):
        """Test ARG returns multiple workspaces even with location specified."""
        mock_arg_client = mock.Mock(spec=ResourceGraphClient)
        mock_response = mock.Mock()
        mock_response.data = [
            {
        ...
        with mock.patch.dict(os.environ):
            ...
            self.assertIn("Multiple Azure Quantum workspaces found", str(context.exception))
            self.assertIn("Please specify subscription id and resource group", str(context.exception))
    
    def test_load_workspace_from_arg_query_exception(self):
        """Test ARG query raises an exception."""
        mock_arg_client = mock.Mock(spec=ResourceGraphClient)
        mock_arg_client.resources.side_effect = Exception("ARG service error")
        ...
        with mock.patch.dict(os.environ):
            ...
            self.assertIn("Failed to query workspace using ARG", str(context.exception))
            self.assertIn("ARG service error", str(context.exception))
    
    def test_load_workspace_from_arg_incomplete_data(self):
        """Test ARG returns incomplete workspace data (missing required fields)."""
        mock_arg_client = mock.Mock(spec=ResourceGraphClient)
        mock_response = mock.Mock()
        mock_response.data = [{
            'subscriptionId': SUBSCRIPTION_ID,
            'resourceGroup': RESOURCE_GROUP,
            # Missing location and endpointUri
            'name': WORKSPACE
        }]
        mock_arg_client.resources.return_value = mock_response
        ...
        with mock.patch.dict(os.environ):
            ...
            self.assertIn("Failed to retrieve complete workspace details", str(context.exception))
            self.assertIn("Please check that workspace is in valid state", str(context.exception))
    
    def test_escape_kql_string(self):
        """Test KQL string escaping for injection prevention."""
        # Test single quote escaping
        self.assertEqual(Workspace._escape_kql_string("test'workspace"), "test\\'workspace")
        
        # Test backslash escaping
        self.assertEqual(Workspace._escape_kql_string("test\\workspace"), "test\\\\workspace")
        
        # Test both backslash and single quote
        self.assertEqual(Workspace._escape_kql_string("test\\'workspace"), "test\\\\\\'workspace")
        
        # Test empty string
        self.assertEqual(Workspace._escape_kql_string(""), "")
        
        # Test None
        self.assertIsNone(Workspace._escape_kql_string(None))
        
        # Test normal string
        self.assertEqual(Workspace._escape_kql_string("normalworkspace"), "normalworkspace")
    
    def test_load_workspace_from_arg_with_special_characters(self):
        """Test workspace discovery with special characters in parameters."""
        workspace_name_with_quote = "test'workspace"
        escaped_name = "test\\'workspace"
        ...
        mock_response.data = [{
            ...
            'location': LOCATION,
            'endpointUri': ENDPOINT_URI,
            'name': workspace_name_with_quote
        }]
        mock_arg_client.resources.return_value = mock_response
        ...
        with mock.patch.dict(os.environ):
            ...
            # Verify escaped name was used in query
            call_args = mock_arg_client.resources.call_args[0][0]
            self.assertIn(escaped_name, call_args.query)
            self.assertNotIn(workspace_name_with_quote, call_args.query)
