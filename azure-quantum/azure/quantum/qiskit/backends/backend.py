...
class AzureBackend(Backend):
    ...
    def _translate_input(self, circuits: Union[QuantumCircuit, List[QuantumCircuit]], data_format, input_params, to_qir_kwargs={}):
        ...
        if isinstance(circuits, QuantumCircuit):
            circuits = [circuits]
        elif isinstance(circuits, List[QuantumCircuit].__origin__) and len(circuits) > 0 and isinstance(circuits[0], QuantumCircuit):
            pass
        else:
    ...
    def run(self, circuit, **kwargs):
        ...
        if isinstance(circuit, (list, tuple)) and len(circuit) == 1:
            circuit = circuit[0]
