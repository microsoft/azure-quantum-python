...
def create_container_using_client(container_client: ContainerClient):
    ...
    try:
        ...
        logger.debug(f"  - uploading to **new** "
                     + f"container: {container_client.container_name}")
        container_client.create_container()


def get_container_uri(connection_string: str, container_name: str) -> str:
    """
    Creates and initialize a container; returns a URI with a SAS read/write
    token to access it.
    """
    container = create_container(connection_string, container_name)
    logger.info(f"Creating SAS token for container '{container_name}' "
...
def upload_blob(
    ...
    """
    Uploads the given data to a blob record. If a blob with the given name
    already exist, it throws an error.
...
def download_blob_metadata(blob_url: str) -> Dict[str, str]:
    """Downloads the blob metadata from the blob properties in Azure for the
    given blob URI"""
...
def set_blob_metadata(blob_url: str, metadata: Dict[str, str]):
    ...
    blob_client = BlobClient.from_blob_url(blob_url)
    logger.info(f"Setting blob properties '{blob_client.blob_name}' from "
...
def init_blob_for_streaming_upload(
    ...
    """
    create_container_using_client(container)
    logger.info(f"Streaming blob '{blob_name}' to "
...
class StreamedBlob:
    """Class that provides a state machine for writing blobs using the
    Azure Block Blob API

    Internally implements a state machine for uploading blob data. To use,
    start calling `upload_data()` to add data blocks.
    Each call to `upload_data()` will synchronously upload an individual block
    to Azure.
    Once all blocks have been added, call `commit()` to commit the blocks and
    make the blob available/readable.

    :param container: The container client that the blob will be uploaded to
    :param blob_name: The name of the blob (including optional path) within
        the blob container
    :param content_type: The HTTP content type to apply to the blob metadata
    :param content_encoding: The HTTP content encoding to apply to the blob
        metadata
    """
    def __init__(
            self,
    ...
    def upload_data(self, data):
        ...
        if self.state == StreamedBlobState.not_initialized:
            ...
            logger.info(f"Streaming blob '{self.blob_name}' to container "
                        + f"'{self.container.container_name}' on account: "
                        + f"'{self.container.account_name}'")
    ...
    def commit(self, metadata: Dict[str, str] = None):
        """Synchronously commits all previously uploaded blobs
        to the block blob.

        :param metadata: Optional dictionary of metadata to be applied to the
            block blob
        """
        if self.state == StreamedBlobState.not_initialized:
    ...
    def getUri(self, with_sas_token: bool = False):
        """Gets the full Azure Storage URI for the uploaded blob after it has
        been committed"""
