...
To install run: pip install azure-quantum[pyscf]"
    )
...
def load_qcschema_mol_scf(qcschema_dict,save_chk=False,chkfile="output.chk",to_Angstrom=False):
    ...
    # QCSchema scf_occupations_a only for R, so occ is 1 or 0.
    ...
    # Need to reshape MO coefficients for PySCF.
    ...
    syms = np.array(qcschema_dict["molecule"]["symbols"])
    geo = np.array(qcschema_dict["molecule"]["geometry"])
    PySCF_atoms = load_qcschema_molecule(qcschema_dict, to_Angstrom, xyz=False)
    ...
    mol = pyscf.gto.Mole(atom=PySCF_atoms,basis=PySCF_basis,charge=PySCF_charge,spin=PySCF_spin,cart=PySCF_cart,unit=units)
