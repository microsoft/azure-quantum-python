...
class SimulatedBifurcationMachine(Solver):
    def __init__(
        ...
        This solver enables users to quickly obtain nearly optimal solutions
        for large combinatorial optimization problems.
        It is developed based on the theory described in the following paper:
        Goto, H., Tatsumura, K., & Dixon, A. R.
        (2019) Combinatorial optimization by simulating adiabatic bifurcations
        in nonlinear Hamiltonian systems,
        Science Advances, 5(4), DOI:10.1126/sciadv.aav2372
        :param steps:
           The number of steps in a computation request.
           The default is 0.
           The value 0 means auto step where SBM computation service
           dynamically determines the number of steps.
           The maximum is 100,000,000.
        :param loops:
            The number of loops in SBM computation.
            SBM computation service searches for a better solution while
            repeating loops as many times as is specified.
            The default is 1. If 0 (zero) is specified, computation will be
            repeated until a timeout occurs. The maximum is 10,000,000.
        :param target_val:
            The end condition of a computation request. When the evaluation
            value reaches this value, the computation will stop.
            If 0 is specified for the parameter loops, loops will be repeated
            either until the objective function reaches the value
            specified in the parameter target or until a timeout occurs.
        :param maxout:
            The upper limit of the number of solutions to be outputted.
            The default is 1.
            Until the limit specified by maxout is reached, SBM computation
            service outputs the obtained solutions in descending order
        ...
        :param timeout:
            ...
            In this case, the execution result will be the best solution
            obtained thus far.
            If 0 is specified for the parameter loops, loops will be repeated
            until the time specified by the parameter timeout expires.
            The maximum is 360.
        :param dt:
            The time per step. The range of the value is greater than 0.0 and
            less than or equal to 1.0.
        :param C:
            Corresponds to the constant ξ0, appearing in the paper
            by Goto, Tatsumura, & Dixon (2019, p. 2), which is the theoretical
            basis of SBM.
            Specify the constant as a single-precision floating point number,
            equal to or more than 0.
            If the value is 0, the value C is automatically determined.
        :param algo:
            The type of SBM computation algorithm. The default is "2.0".
            One of "1.5" or "2.0".
            Depending on the type of problem, there may be differences in
            performance between the "1.5" and "2.0" algorithms.
            Try both and decide which yields better performance.
        :param auto:
            The parameter auto tuning flag. The default is "false."
            If the value is "true," SBM computation service searches for the
            values of the parameters automatically to obtain the best solution.
            Parameters other than `auto` are treated as follows in this case.
            `algo` and `dt` are ignored and tuned automatically.
            `loops` and `maxout` are ignored. `timeout` can be specified as
            the total computation time (sec).
    ...
    # solver to mitigate an issue downloading gzip files using
    # the Blob Storage SDK
