...
try:
    ...
    def _convert_if_numpy_type(param: WArray):
        # Attempt first a conversion to a supported type if parameter
        # is a numpy float/int.
        numpy_integer_types = [np.byte, np.ubyte, np.short, np.ushort, np.intc,
                               np.uintc, np.int_, np.uint, np.longlong,
                               np.ulonglong, np.int8, np.uint8, np.int16,
                               np.uint16, np.int32, np.uint32, np.int64,
                               np.uint64]

        numpy_float_types = [np.float16, np.float32, np.float64, np.float_,
...
class Term:
    ...
    def __init__(
        ...
        if w is not None:
            # Legacy support if 'w' is used to specify term instead of
            # the expected 'c'.
            coeff = w
        ...
        if type(coeff) != int and type(coeff) != float:
            raise RuntimeError(f"{parameter_name_used} must be a float or int \
                                value, or a NumPy value that can be converted \
                                to those.")
        self.c = coeff
    ...
    def evaluate(self, configuration: Dict[int, int]) -> float:
        ...
        try:
            ...
            print("Error - variable id found in term {0}, but not found in \
                   the supplied configuration.".format(self.ids))
            raise
    ...
    def reduce_by_variable_state(
        ...
        """ Given some fixed variable states, transform the existing term
            into new term.
