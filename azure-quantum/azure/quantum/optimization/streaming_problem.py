...
from azure.quantum.storage import (StreamedBlob,
                                   ContainerClient,
                                   BlobClient,
                                   download_blob)
...
class StreamingProblem(object):
    ...
    def add_term(self, c: Union[int, float], indices: List[int]):
        """Adds a single term to the `Problem` representation and queues it
        to be uploaded.
    ...
    def add_terms(self, terms: List[Term]):
        """Adds a list of terms to the `Problem` representation and queues
        them to be uploaded.
        ...
        if terms is not None:
            if self.uploader is None:
                ...
                raise Exception('Cannot add terms after problem has been \
                                 uploaded')
    ...
    def download(self):
        ...
        if not self.uploaded_uri:
            raise Exception('StreamingProblem may not be downloaded before \
                             it is uploaded')
...
class JsonStreamingProblemUploader:
    ...
    :param container: Reference to the container client in which to
        store the problem
    ...
    :param upload_term_threshold: Chunking threshold (in terms).
        Once this many terms are ready to be uploaded, the chunk
        will be uploaded.
    ...
    def __init__(
        ...
        self.compressor = (gzip.GzipFile(mode='wb',
                                         fileobj=self.compressedStream)
                           if compress
                           else None)
    ...
    def start(self):
        ...
        if self.__thread is not None:
            raise Exception('JsonStreamingProblemUploader thread \
                             already started')
    ...
    def join(self, timeout: float = None) -> StreamedBlob:
        """Joins the problem uploader thread - returning when it completes
        ...
        while continue_processing:
            try:
                new_terms = self.problem.terms_queue.get(
                                block=True,
                                timeout=self.__queue_wait_timeout)
    ...
    def _get_terms_string(self, terms):
        result = (("," if self.uploaded_terms > 0 else "")
                  + ",".join([json.dumps(term.to_dict()) for term in terms]))
