...
class Job(BaseJob, FilteredJob):
    ...
    def __init__(self, workspace: "Workspace", job_details: JobDetails, **kwargs):
        self.results = None
        super().__init__(workspace=workspace, details=job_details, **kwargs)
    ...
    def has_succeeded(self) -> bool:
        ...
        return self.details.status == "Completed" or self.details.status == "Succeeded"

    def wait_until_completed(
        self, max_poll_wait_secs=30, timeout_secs=None, print_progress=True
    ) -> None:
        ...
        while not self.has_completed():
            if timeout_secs is not None and (time.time() - start_time) >= timeout_secs:
                raise TimeoutError(
                    f"The wait time has exceeded {timeout_secs} seconds."
                )
    ...
    def get_results(self, timeout_secs: float = DEFAULT_TIMEOUT):
        ...
        if not self.has_succeeded():
            if self.details.status == "Failed" and self._allow_failure_results():
                job_blob_properties = self.download_blob_properties(
                ...
                if job_blob_properties.size > 0:
                    ...
                    raise JobFailedWithResultsError(
                        "An error occurred during job execution.", job_failure_data
                    )
        ...
        try:
            ...
            if self.details.output_data_format == "microsoft.quantum-results.v1":
                if "Histogram" not in results:
                    raise ValueError(
                        f'"Histogram" array was expected to be in the Job results for "{self.details.output_data_format}" output format.'
                    )
                ...
                if len(histogram_values) % 2 == 0:
                    ...
                    return {
                        histogram_values[i]: histogram_values[i + 1]
                        for i in range(0, len(histogram_values), 2)
                    }
                else:
                    raise ValueError(
                        f'"Histogram" array has invalid format. Even number of items is expected.'
                    )
            elif self.details.output_data_format == "microsoft.quantum-results.v2":
                if (
                    "DataFormat" not in results
                    or results["DataFormat"] != "microsoft.quantum-results.v2"
                ):
                    raise ValueError(
                        f'"DataFormat" was expected to be "microsoft.quantum-results.v2" in the Job results for "{self.details.output_data_format}" output format.'
                    )

                if "Results" not in results:
                    raise ValueError(
                        f'"Results" field was expected to be in the Job results for "{self.details.output_data_format}" output format.'
                    )

                if len(results["Results"]) < 1:
                    raise ValueError(
                        '"Results" array was expected to contain at least one item'
                    )
                ...
                return {
                    outcome["Display"]: outcome["Count"] / total_count
                    for outcome in histogram_values
                }
            ...
            if self.details.output_data_format == "microsoft.quantum-results.v2":
                ...
                if len(results) == 1:
                    ...
                    return {
                        hist_val["Display"]: {
                            "outcome": outcome,
                            "count": hist_val["Count"],
                        }
                        for outcome, hist_val in zip(outcome_keys, histogram_values)
                    }
                    ...
                    for i, result in enumerate(results):
                        if "Histogram" not in result:
                            raise ValueError(
                                f'"Histogram" array was expected to be in the Job results for result {i} for "{self.details.output_data_format}" output format.'
                            )
                        ...
                        resultsArray.append(
                            {
                                ...
                                for outcome, hist_val in zip(
                                    outcome_keys, histogram_values
                                )
                            }
                        )
                ...
                raise ValueError(
                    f'Getting a results histogram with counts instead of probabilities is not a supported feature for jobs using the "{self.details.output_data_format}" output format.'
                )
            ...
            if self.details.output_data_format == "microsoft.quantum-results.v2":
                ...
                if len(results) == 1:
                    ...
                    for i, result in enumerate(results):
                        ...
                        shotsArray.append(
                            [self._convert_tuples(shot) for shot in result["Shots"]]
                        )

                    return shotsArray
            else:
                raise ValueError(
                    f'Individual shot results are not supported for jobs using the "{self.details.output_data_format}" output format.'
                )
        except Exception as e:
            raise e
    ...
    def _convert_tuples(self, data):
        if isinstance(data, dict):
            ...
            # Check if the dictionary represents a tuple
                ...
                return tuple(
                    self._convert_tuples(data[f"Item{i+1}"]) for i in range(len(data))
                )
            else:
