...
if TYPE_CHECKING:
    ...
    from azure.quantum.aio_workspace import AsyncWorkspace
...
class AsyncJob(Job):
    """Azure Quantum Job that is submitted to a given AsyncWorkspace.

    :param workspace: AsyncWorkspace instance to submit job to
    :type workspace: AsyncWorkspace
    :param job_details: Job details model,
            contains Job ID, name and other details
    :type job_details: JobDetails
    """

    def __init__(self, workspace: "AsyncWorkspace", job_details: JobDetails):
        self.workspace = workspace
        self.details = job_details
        self.id = job_details.id
        self.results = None
    ...
    async def _download_blob(self, blob_url):
        blob_client = AsyncBlobClient.from_blob_url(blob_url)
        logger.info(
