...
class Job:
    ...
    :param job_details: Job details model, contains Job ID, name and other
    details
    ...
    def wait_until_completed(self, max_poll_wait_secs=30):
        """Keeps refreshing the Job's details until it reaches a finished
        status.
        """
        self.refresh()
        poll_wait = 0.2
    ...
    def get_results(self):
        ...
        if not self.details.status == "Succeeded":
            raise RuntimeError(f"Cannot retrieve results as job execution "
                               + f"failed (status: {self.details.status}. "
