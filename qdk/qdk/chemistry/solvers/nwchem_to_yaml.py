# Code from /opt/chem/nwchem/contrib/quasar/export_chem_library_yaml.py
...
Quantum/master/Chemistry/Schema/broombridge-0.1.schema.json
"""
...
=======================":
            ...
            #        fci_val = float(ln.split("=")[1].strip())
            #        fci_energy = {"units": "hartree", "value": fci_val,
            #   "upper": fci_val*0.99, "lower":fci_val*1.01}
            elif ln == "Ground state specification:":
                reader_mode = "initial_state"
                assert ccsd_energy is not None, \
                    "CCSD energy should be available before the groud state \
                    specification."
                if initial_state is None:
                    initial_state = []
                initial_state += [{'state': {
                        'label': '|G>',
                        'energy': {'units': 'hartree', 'value': ccsd_energy},
                        'superposition': []
                        }}]
            elif ln == "Excited state specification:":
                reader_mode = "initial_state"
                assert ccsd_energy is not None, \
                    "CCSD energy should be available before the excited state \
                        specification."
                if initial_state is None:
                    initial_state = []
                initial_state += [{'state': {
            ...
            if ln == "-------------------------------------":
                ...
                # vals = ln.split(":")
                if len(initial_state[-1]['state']['superposition']) > 0 and \
                    initial_state[-1]['state']['superposition'][-1][-1] != \
                        '|vacuum>':
                    initial_state[-1]['state']['superposition'][-1] += \
                        ln.replace('|0>', '|vacuum>').split()
                else:
...
=======================":
            ...
            elif skip_input_geometry is False:
                # atom description line
                if (
                    len(ln_segments) != 4 or not is_float(ln_segments[1])
                    or not is_float(ln_segments[2])
                    or not is_float(ln_segments[3])
                ):
    ...
    if fci_energy is None:
        ...
        fci_energy = {
            "units": "hartree",
            "value": ccsd_energy,
            "upper": ccsd_energy + 0.1,
            "lower": ccsd_energy - 0.1
        }

    assert one_electron_integrals is not None, \
        "one_electron_integrals is missing from NWChem output. \
            Required to extract YAML"
    assert two_electron_integrals is not None, \
        "two_electron_integrals is missing from NWChem output. \
            Required to extract YAML"
    assert geometry is not None, \
        "geometry information is missing from NWChem output. \
            Required to extract YAML"
    assert basis_set is not None, \
        "basis_set is missing from NWChem output. \
            Required to extract YAML"
    assert coulomb_repulsion is not None, \
        "coulomb_repulsion is missing from NWChem output. \
            Required to extract YAML"
    assert scf_energy is not None, \
        "scf_energy is missing from NWChem output. \
            Required to extract YAML"
    assert scf_energy_offset is not None, \
        "scf_energy_offset is missing from NWChem output. \
            Required to extract YAML"
    assert energy_offset is not None, \
        "energy_offset is missing from NWChem output. \
            Required to extract YAML"
    assert fci_energy is not None, \
        "fci_energy is missing from NWChem output. \
            Required to extract YAML"
    assert n_orbitals is not None, \
        "n_orbitals is missing from NWChem output. \
            Required to extract YAML"
    assert n_electrons_alpha is not None, \
        "n_electrons_alpha is missing from NWChem output. \
            Required to extract YAML"
    assert n_electrons_beta is not None, \
        "n_electrons_beta is missing from NWChem output. \
            Required to extract YAML"
...
def emitter_ruamel_func(file_path) -> str:
...
def emitter_yaml_func(file_path) -> str:
    response = preamble
    data = extract_fields(file_path)
...
def to_broombridge(file_path: str) -> str:
    assert emitter_yaml or emitter_ruamel, "Extraction failed: could not \
        import YAML or RUAMEL packages."
    if emitter_yaml:
        return emitter_yaml_func(file_path)
    elif emitter_ruamel:
        return emitter_ruamel_func(file_path)
    else:
        assert False, "Unreachable code"


def save_broombridge(input_file_path: str, output_file_path: str):
    data = to_broombridge(input_file_path)
