...
function DonutChart({
  ...
  React.useEffect(() => {
    ...
    const legend = svg
      ...
      .attr(
        "transform",
        (_d, i) => `translate(${i * translationValX}, ${translationValY})`
      );
    ...
    legend
      ...
      .style("fill", (d, _i) => chartColor(d.title) as string);

    // add text to the legend
    legend
      .append("text")
      .text((_d) => legendTitlePreText)
      .attr("class", "legend")
      .attr("x", 0)
      .attr("y", translationValY + outerRadius)
      .attr("transform", (_d, _i) => `translate(${translationValX}, 0)`);
